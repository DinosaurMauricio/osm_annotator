...
import math
import pandas as pd
from general_utils import check_closed_way, get_center_way, lat_lon_to_pixel_coords, check_point_inside_bbox, is_represented
...
def filter_tags_in_list(tags_to_keep:list[str], elements:list[dict])->list[dict]:
    '''
    This function filters the tags in the list of elements based on the tags to keep. 
    '''
    new_elements = []
    for element in elements:
        tags = element["tags"]
...
def intersect_elements(filtered_elements:list[dict], image_bbox:list[float]) -> list[dict]:
    ...
    for element in filtered_elements:
        if element["type"]=="node":
            ...
            intersected, duplicate = find_intersection(image_bbox, element)
            if not duplicate:
                intersected_elements.append(intersected)
            else:
                print("Duplicating!")
                for polygon in intersected["intersection"]:
                    new_element = {"type":"way", "tags":intersected["tags"], "nodes":intersected["nodes"], "geometry":intersected["geometry"], "intersection":polygon}
                    intersected_elements.append(new_element)
...
def quantize_boundary(boundary:list[list[float]], image_resolution:int, image_bbox:list[float])-> list[list[int]]:
    ...
    for i, point in enumerate(boundary):
        ...
        pixel_coords = lat_lon_to_pixel_coords(image_bbox, lat, lon, (image_resolution,image_resolution))
        if(pixel_coords is not None):
            pixel_coords_x = int(pixel_coords[0])/image_resolution
            pixel_coords_y = int(pixel_coords[1])/image_resolution
            if i==0:
                quantized_boundary.append((round(pixel_coords_x,2), round(pixel_coords_y,2)))
            else:
                if quantized_boundary[-1] != (round(pixel_coords_x,2), round(pixel_coords_y,2)):
                    quantized_boundary.append((round(pixel_coords_x,2), round(pixel_coords_y,2)))
...
def connect_elements(elements:list[dict])->list[dict]:
    '''
    This function connects elements based on their nodes. 
    This function connects elements that have an end point or starting point that is equal to the end point or starting point of another element, and share the same tags.
    '''
    elements_to_connect = [element for element in elements if element["type"]=="way" and not check_closed_way(element)]
    indexes = [i for i, element in enumerate(elements) if element["type"]=="way" and not check_closed_way(element)]
    if len(elements_to_connect)>1:
        i = 0
        while len(elements_to_connect)>1:
            indices = list(range(len(elements_to_connect)))
            combinations = list(itertools.combinations(indices, 2))
            if i<len(combinations):
                combination = combinations[i]
                el = elements_to_connect[combination[0]]
                el2 = elements_to_connect[combination[1]]
                if str(el["tags"])==str(el2["tags"]):
                    if el["nodes"][0]==el2["nodes"][-1]:
                        nodes = el2["nodes"]+el["nodes"][1:]
                        geometry = el2["geometry"]+el["geometry"][1:]
                        new_element = {"type":"way", "tags":el["tags"], "nodes":nodes, "geometry":geometry}
                        for index in sorted(list(combination), reverse=True):
                            del elements_to_connect[index]
                        elements_to_connect.append(new_element)
                        i = 0
                        
                    elif el["nodes"][-1]==el2["nodes"][0]:
                        nodes = el["nodes"]+el2["nodes"][1:]
                        geometry = el["geometry"]+el2["geometry"][1:]
                        new_element = {"type":"way", "tags":el["tags"], "nodes":nodes, "geometry":geometry}
                        for index in sorted(list(combination), reverse=True):
                            del elements_to_connect[index]
                        elements_to_connect.append(new_element)
                    ...
                    elif el["nodes"][-1]==el2["nodes"][-1]:
                        # Rotate the second element
                        el2["nodes"] = el2["nodes"][::-1]
                        el2["geometry"] = el2["geometry"][::-1]
                        nodes = el["nodes"]+el2["nodes"][1:]
                        geometry = el["geometry"]+el2["geometry"][1:]
                        new_element = {"type":"way", "tags":el["tags"], "nodes":nodes, "geometry":geometry}
                        for index in sorted(list(combination), reverse=True):
                    ...
                    elif el["nodes"][0]==el2["nodes"][0]:
                        # Rotate the first element
                        el["nodes"] = el["nodes"][::-1]
    ...
    elements += elements_to_connect
    
    return elements

def filter_outside_elements(elements:list[dict], image_bbox:list[float])->list[dict]:
    '''
    This function filters objects that lies outside the 97% of the image bounding box.
    '''
    lat_width = image_bbox[2] - image_bbox[0]
    lon_width = image_bbox[3] - image_bbox[1]
    bbox_reduced = [image_bbox[0]+0.03*lat_width, image_bbox[1]+0.03*lon_width, image_bbox[2]-0.03*lat_width, image_bbox[3]-0.03*lon_width]
    bbox_reduced_poly = Polygon([(bbox_reduced[0], bbox_reduced[1]), (bbox_reduced[0], bbox_reduced[3]), (bbox_reduced[2], bbox_reduced[3]), (bbox_reduced[2], bbox_reduced[1]), (bbox_reduced[0], bbox_reduced[1])])
    new_elements = []
    for element in elements:
        # Take the intersection
        if element["type"]=="node":
            point = Point(element["lat"], element["lon"])
            if bbox_reduced_poly.contains(point):
                new_elements.append(element)
        if element["type"]=="way":
            if check_closed_way(element):
                ...
                if len(lat)==1:
                    ...
                    new_intersection = []
                    for i in range(len(element["intersection"])):
                        point = Point(lat[i], lon[i])
                        if bbox_reduced_poly.contains(point):
                            new_intersection.append(element["intersection"][i])
                            
                    if len(new_intersection)!=0:
                        new_element = {"type":"way", "tags":element["tags"], "nodes":element["nodes"], "intersection":new_intersection, "geometry":element["geometry"]}
                        new_elements.append(new_element)
            else:
                # Average the points of the way
                lat = [point["lat"] for point in element["intersection"]]
                lon = [point["lon"] for point in element["intersection"]]
                lat = sum(lat)/len(lat)
                lon = sum(lon)/len(lon)
                point = Point(lat, lon)
                if bbox_reduced_poly.contains(point):
...
def convert_elements_list_to_dict(elements:list[dict])-> dict[dict]:
    '''
    This function converts the list of elements into a dictionary, where each key is the single object identifier
    '''
    element_dict = {}
    for element in elements:
        tags = element["tags"]
        if "geometry" in element.keys():
            object_identifier = str(tags)+" "+str(element["geometry"][0])
        else:
            object_identifier = str(tags)+" "+str(element["id"])
        
        element_dict[object_identifier] = element
...
def post_process_osm_data(base_folder_dataset:str):
    ...
    if not os.path.exists(os.path.join(base_folder_dataset, "postprocessed_osm_data")):
        os.mkdir(os.path.join(base_folder_dataset, "postprocessed_osm_data"))
    ...
    for file in os.listdir(raw_data_path):
        if file.endswith(".json"):
            ...
            elements = filter_outside_elements(elements, image_bbox)
            ...
            elements = convert_elements_list_to_dict(elements)
            if len(elements) != 0:
...
if __name__=="__main__":
    post_process_osm_data("/media/data_fast/Riccardo/osm_data/")
    
    
#cp -a /media/data_fast/Riccardo/osm_project_data/osm_data/. /media/data_fast/Riccardo/test_osm_scraping/raw_osm_data/